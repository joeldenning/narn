...
// like yarn add -D a ==> yarn add a -D, yarn add --dev a ==> yarn add a --dev
let reverseOption = function reverseOption(narnArgs) {
  const reg = /^-{1,2}.+$/;
  return narnArgs
    .filter(v => !reg.test(v))
    .concat(narnArgs.filter(v => reg.test(v)));
};
...
exports.getNpmArgs = narnArgs => {
  const yarnArgs = minimist(reverseOption(narnArgs));
  ...
  switch (yarnTarget) {
    ...
    case "add":
      npmTarget = "install";
      const packages = yarnArgs.dev
        ? [transformNarnPackageString(yarnArgs.dev)]
        : [];
