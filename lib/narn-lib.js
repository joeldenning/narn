...
exports.detectYarn = function detectYarn(args) {
  if (args && args.length > 0 && args[0] === 'global') {
    ...
    return new Promise((resolve, reject) => {
      fs.access(path.resolve(process.cwd(), 'yarn.lock'), fs.constants.F_OK, noYarnLock => {
...
exports.detectPnpm = function detectNpm(args) {
  if (args && args.length > 0 && args[0] === 'global') {
    ...
    return new Promise((resolve, reject) => {
      fs.access(path.resolve(process.cwd(), 'pnpm-lock.yaml'), fs.constants.F_OK, noPnpmLock => {
...
function transformNarnPackageString(packageToAdd) {
  ...
  const packageNameWithoutVersion = packageToAdd.slice(
    0,
    packageToAdd.indexOf('@') > 0 ? packageToAdd.lastIndexOf('@') : packageToAdd.length + 1
  );
  const isValidPackageName = validateNpmPackageName(packageNameWithoutVersion).validForNewPackages;
